...
impl LayoutConfig {
    ...
    pub fn alloc_low<T: ToString>(&mut self, name: T, length: usize, entry_type: T) {
        self.list.insert(
            self.free,
            LayoutEntry::new(
                name.to_string(),
        ...
        self.low_offset = self.low_offset.checked_add(length).expect("Invalid ");
    ...
    pub fn alloc_high<T: ToString>(&mut self, name: T, length: usize, entry_type: T) {
...
mod test {
    ...
    fn test_basic() {
        ...
        regions.alloc_low("BOOTLOADER", 0x80_0000, "Memory");
        regions.alloc_low("TD_HOB", 0x10_0000, "Memory");
        regions.alloc_low("LAZY_ACCEPT_BITMAP", 0x4_0000, "Memory");
        regions.alloc_low("TD_PAYLOAD_PARAM", 0x1000, "Memory");
        regions.alloc_low("TD_PAYLOAD", 0x200_0000, "Memory");
        regions.alloc_high("ACPI", 0x10_0000, "Acpi");
        regions.alloc_high("PAYLOAD", 0x200_0000, "Reserved");
        regions.alloc_high("PAGE_TABLE", 0x2_0000, "Reserved");
        regions.alloc_high("STACK", 0x10000, "Reserved");
        regions.alloc_high("MAILBOX", 0x2000, "Nvs");
        regions.alloc_high("TD_EVENT_LOG", 0x10_0000, "Nvs");
    }
}
